...
plot_path <- function(x, p_x, p_x_tilde, ranges, highlight, epsilon = seq(0, 1, 0.1)) {
  ...
  lines(
    x,
    (1 - highlight) * p_x + highlight * p_x_tilde,
    col = cols[highlight * 10],
    lwd = 2,
    lty = 1
  )
}
...
plot_curve <- function(x, y, dydx, legend_pos, title="", xlbl="epsilon") {
